## AMSWireDAQ
To host the Dockefile for a container with ROOT, compiler, emacs, svn, ..., so that you can compile and use AMSWireDAQ

The code on GitHub (https://github.com/bozzochet/DockerAMSWireDAQ) is then connected to DockerHub (https://github.com/bozzochet/amswiredaq). The idea is to take the Docker image and run directly from DockerHub without looking at the GitHub code.
...
```
$ docker build --tag bozzochet/amswiredaq:latest .
```
...
```
docker pull bozzochet/amswiredaq
```
...
```
docker run --rm -it bozzochet/amswiredaq:latest
```
...
```
docker run --rm -it -v `pwd`:/home/studente/current_dir bozzochet/amswiredaq:latest
```
...
```
docker run --net=host --rm -it bozzochet/amswiredaq:latest
```

To enter the container as root (there's no sudo for the default user, so no package installing allowed):
```
docker run --user root --rm -it bozzochet/amswiredaq:latest
```

**The suggested way of running, to allow also the creation of the lxplus ssh socket, is**:
```
docker run --net=host --user root --rm -it -v `pwd`:/home/testsys/current_dir bozzochet/amswiredaq:latest
```
...
```
docker run -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix --rm -it --user $(id -u) bozzochet/amswiredaq:latest
```
...
```
docker run --rm -it -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$ip:0 bozzochet/amswiredaq:latest
```

## Examples (OSX as "host")
##### Bash session:
```
docker run --rm -it -v /tmp/.X11-unix:/tmp/.X11-unix -v `pwd`:/home/studente/current_dir -e DISPLAY=`ifconfig en0 | grep inet | awk '$1=="inet" {print $2}'`:0 bozzochet/amswiredaq:latest /bin/bash
```
will
  - export the display;
...
```
docker run --rm -it -v /tmp/.X11-unix:/tmp/.X11-unix -v `pwd`:/home/studente/current_dir -e DISPLAY=`ifconfig en0 | grep inet | awk '$1=="inet" {print $2}'`:0 bozzochet/amswiredaq:latest
```
will
  - export the display;
  - export the current dir (i.e. `pwd`) to the container 'current_dir' inside the user (i.e. 'studente') home. This is quite useful and makes the bash and ROOT history persistent;
...
```
docker run --rm -it -v /tmp/.X11-unix:/tmp/.X11-unix -v `pwd`:/home/studente/current_dir -e DISPLAY=`ifconfig en0 | grep inet | awk '$1=="inet" {print $2}'`:0 bozzochet/amswiredaq:latest root
```
will
  - export the display;
  - export the current dir (i.e. `pwd`) to the container 'current_dir' inside the user (i.e. 'studente') home. This is quite useful and makes the bash and ROOT history persistent;
...
```
docker run --rm -it -v /tmp/.X11-unix:/tmp/.X11-unix -v `pwd`:/home/studente/current_dir -e DISPLAY=`ifconfig en0 | grep inet | awk '$1=="inet" {print $2}'`:0 bozzochet/amswiredaq:latest root macro.C
```
will
  - export the display;
